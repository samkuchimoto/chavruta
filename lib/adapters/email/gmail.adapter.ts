/**
 * lib/adapters/email/gmail.adapter.ts
 *
 * Gmail OAuth2 + Nodemailer implementation of EmailPort.
 *
 * Requires: GMAIL_CLIENT_ID, GMAIL_CLIENT_SECRET, GMAIL_REFRESH_TOKEN,
 *           GMAIL_FROM_ADDRESS (all checked in lib/config.ts before this
 *           adapter is selected in the container).
 *
  ...
  private async send(to: string, subject: string, html: string, text: string): Promise<void> {
    try {
      await this.getTransporter().sendMail({
        from:    `"Chavruta" <${process.env.GMAIL_FROM_ADDRESS}>`,
        to, subject, html, text,
      })
    } catch (err) {
      console.error(`[gmail] send failed "${subject}" → ${to}:`, err)
    }
  }
